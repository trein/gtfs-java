 ...
 * It also allows states to be retrieved for a given target vertex. Different implementations of
 * this interface allow the routing algorithm to switch from a basic Dijkstra search (which is
 * sufficient for non-time-dependent modes e.g. bicycle) and the more complex label-setting
...
public interface ShortestPathTree {
     ...
     * relaxed. Note: This is necessary because OTP priority queues are not required to implement
     * the decrease-key operation. In fact, decrease-key is not relevant when it is possible to have
