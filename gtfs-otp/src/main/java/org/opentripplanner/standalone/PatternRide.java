...
class PatternRide {
    ...
    public boolean equals(Object o) {
        ...
        if (other.fromIndex != this.fromIndex) { return false; }
        if (other.toIndex != this.toIndex) { return false; }
    ...
    public String toString() {
        return String.format("%s %d to %d", this.pattern.code, this.fromIndex, this.toIndex);
    }
    
    /**
     * Complete an unfinished ride as a new object. Perhaps we should check in advance whether a
     * pattern is running at all, but results will vary depending on the fromIndex and toIndex. Some
     * indexes can be reached within the window, others not. But that's an optimization. It's
     * actually advantageous to call this sequentially for each to-stop instead of incrementally
     * along a pattern. This is because we store the absolute times of arrivals and departures, and
     * by subtracting those we get accurate min and max ride times rather than looser lower and
     * upper bounds on travel time.
     */
    public PatternRide extendToIndex(int toIndex, TimeWindow window) {
        Stats stats = Stats.create(this.pattern, this.fromIndex, toIndex, window);
        /* There might not be any trips within the time window. */
        if (stats == null) { return null; }
        /*
         * Copy most fields from this unfinished ride, setting the other fields to complete the
         * ride.
         */
