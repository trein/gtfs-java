 ...
 * that are the union of many circles. The origin snapping option used here (connect to all stations
 * within M meters of a lat/lon) should be made into a standard query option in OTP. Instead of
 * specifying an origin station search radius, it could be more coherent to use all stations within
 * (maxIsochroneTime * speed) meters, and apply a travel time to reach them. The GET methods both
 * use makeContours which in turn uses makePoints (where range checking occurs). Putting "@Setter"
 * on all the parameters would allow resource classes to be used outside Jersey.
 */
...
public class SimpleIsochrone extends RoutingResource {
    ...
    private Map<Vertex, Double> makePoints() throws Exception {
        ...
        this.request = buildRequest(0);
        Graph graph = otpServer.graphService.getGraph();
        // double speed = request.getWalkSpeed();
        Coordinate originCoord = this.request.from.getCoordinate();
        if (originCoord == null) { return null; }
        List<TransitStop> stops = graph.streetIndex.getNearbyTransitStops(originCoord, this.radiusMeters);
        if (stops.isEmpty()) {
            LOG.error("No stops found within {} meters.", this.radiusMeters);
            return null;
        }
        if (this.shpName == null) {
            this.shpName = stops.get(0).getName().split(" ")[0];
        }
        ...
        this.request.setRoutingContext(graph, origin, null);

        /* Make one request every M minutes over H hours */
        int nRequests = (this.requestTimespanHours * 60) / this.requestSpacingMinutes;
        this.request.clampInitialWait = (this.requestSpacingMinutes * 60);
        Date date = this.request.getDateTime();
        MinMap<Vertex, Double> points = new MinMap<Vertex, Double>();
        for (int r = 0; r < nRequests; r++) {
            this.request.dateTime = (date.getTime() / 1000) + (r * this.requestSpacingMinutes * 60);
            LOG.debug("date: {} {}", new Date(this.request.dateTime), this.request.dateTime);
            ShortestPathTree spt = sptService.getShortestPathTree(this.request, 10);
            /* This could even be a good use for a generic SPT merging function */
            for (State s : spt.getAllStates()) {
                if (this.stopsOnly && !(s.getVertex() instanceof TransitStop)) {
                    continue;
                }
    ...
    private Map<Integer, Geometry> makeContours() throws Exception {
        ...
        for (int c = 0; c < this.nContours; ++c) {
            int thresholdSeconds = (c + 1) * this.contourSpacingMinutes * 60;
            for (Map.Entry<Vertex, Double> vertexSeconds : points.entrySet()) {
                ...
                if (remainingSeconds > 60) { // avoid degenerate geometries
                    double remainingMeters = remainingSeconds * this.request.walkSpeed;
