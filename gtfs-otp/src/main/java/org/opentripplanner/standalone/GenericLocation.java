 ...
 * through. Handles parsing of geospatial information from strings so that it need not be littered
 * through the routing code.
 *
 * @author avi
 */
public class GenericLocation implements Cloneable {
    ...
    /**
     * Observed heading if any. Direction of travel in decimal degrees from -180° to +180° relative
     * to true north. 0 = heading true north. +/-180 = heading south.
     */
    ...
    private static final Pattern _latLonPattern = Pattern.compile("[^[\\d&&[-|+|.]]]*(" + _doublePattern + ")(\\s*,\\s*|\\s+)("
            + _doublePattern + ")\\D*");

    private static final Pattern _headingPattern = Pattern.compile("\\D*heading=(" + _doublePattern + ")\\D*");
