...
public class PlanGenerator {
    ...
    public TripPlan generate(RoutingRequest options) {
        ...
        try {
            paths = this.pathService.getPaths(options);
    ...
    public Itinerary generateItinerary(GraphPath path, boolean showIntermediateStops) {
        if (path.states.size() < 2) { throw new TrivialPathException(); }
    ...
    private void addFrequencyFields(State[] states, Leg leg) {
         ...
         * FrequencyBoard) { State preBoardState = states[0].getBackState(); FrequencyBoard fb =
         * (FrequencyBoard) states[0].getBackEdge(); FrequencyBasedTripPattern pt = fb.getPattern();
         * int boardTime; if (preBoardState.getServiceDay() == null) { boardTime = 0; //TODO why is
         * this happening? } else { boardTime = preBoardState.getServiceDay().secondsSinceMidnight(
         * preBoardState.getTimeSeconds()); } int period = pt.getPeriod(fb.getStopIndex(),
         * boardTime); //TODO fix leg.isNonExactFrequency = !pt.isExact(); leg.headway = period; }
         */
    }
    ...
    public static List<WalkStep> generateWalkSteps(State[] states, WalkStep previous) {
        ...
        for (int i = 0; i < (states.length - 1); i++) {
            ...
            } else if ((((step.streetName != null) && !step.streetNameNoParens().equals(streetNameNoParens)) && (!step.bogusName || !edge
                    .hasBogusName()))
    ...
    private static List<P2<Double>> encodeElevationProfile(Edge edge, double offset) {
        if (!(edge instanceof EdgeWithElevation)) { return new ArrayList<P2<Double>>(); }
        EdgeWithElevation elevEdge = (EdgeWithElevation) edge;
        if (elevEdge.getElevationProfile() == null) { return new ArrayList<P2<Double>>(); }
    ...
    public TripPlan generateFirstTrip(RoutingRequest request) {
        Graph graph = this.graphService.getGraph(request.routerId);
