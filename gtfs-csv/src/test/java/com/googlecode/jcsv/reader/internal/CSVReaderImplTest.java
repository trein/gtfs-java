...
public class CSVReaderImplTest {
	...
	List<Person> result = this.csvReader.readAll();
	List<Person> expected = new ArrayList<Person>() {
	    {
		add(new Person("Hans", "im \"Gluck\"", 16));
	...
	Person result = this.csvReader.readNext();
	Person expected = new Person("Hans", "im \"Gluck\"", 16);
	assertEquals(expected, result);
	
	result = this.csvReader.readNext();
	...
	List<String> result = this.csvReader.readHeader();
