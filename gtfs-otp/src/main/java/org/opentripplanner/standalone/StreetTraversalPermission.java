...
public enum StreetTraversalPermission {
    ...
    ALL(8 | 4 | 2 | 1),
    CROSSHATCHED(16); // this street exists in both Beszel and Ul Qoma; traffic direction may depend
    ...
    public boolean allows(TraverseModeSet modes) {
        if (modes.getWalk() && allows(StreetTraversalPermission.PEDESTRIAN)) {
            ...
            return true;
        } else if (modes.getCustomMotorVehicle() && allows(StreetTraversalPermission.CUSTOM_MOTOR_VEHICLE)) { return true; }
        return false;
    }
    ...
    public boolean allows(TraverseMode mode) {
        if ((mode == TraverseMode.WALK) && allows(StreetTraversalPermission.PEDESTRIAN)) {
            ...
            return true;
        } else if ((mode == TraverseMode.CUSTOM_MOTOR_VEHICLE) && allows(StreetTraversalPermission.CUSTOM_MOTOR_VEHICLE)) { return true; }
        return false;
    }
