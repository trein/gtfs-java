...
public class StreetLocation extends StreetVertex {
    ...
    private final ArrayList<Edge> extra = new ArrayList<Edge>();
     ...
     */
    public static StreetLocation createStreetLocation(Graph graph, String label, String name, Iterable<StreetEdge> edges,
            ...
            if (street instanceof AreaEdge) {
                for (StreetEdge e : ((AreaEdge) street).getArea().getEdges()) {
                    if (!allEdges.contains(e)) {
                        ...
                        StreetLocation areaSplitter = createStreetLocation(graph, label, name, Arrays.asList(e),
     ...
     */
    public static StreetLocation createStreetLocationOnEdges(Graph graph, String label, String name, Iterable<StreetEdge> edges,
        ...
        for (StreetEdge street : edges) {
            ...
            double distanceToNearestTransitStop = Math.min(tov.getDistanceToNearestTransitStop(), fromv
    ...
    private static void createHalfLocation(Graph graph, StreetLocation base, String label, String name, Coordinate nearestPoint,
            StreetEdge street) {
    ...
    public void addExtraEdgeTo(Vertex target) {
        this.extra.add(new FreeEdge(this, target));
        this.extra.add(new FreeEdge(target, this));
    }
    ...
    public int removeTemporaryEdges() {
        int nRemoved = 0;
        for (Edge e : getExtra()) {
            this.graph.removeTemporaryEdge(e);
