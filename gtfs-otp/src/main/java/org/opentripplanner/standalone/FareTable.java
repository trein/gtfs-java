...
import org.opentripplanner.standalone.DCFareCalculator.Fare;
...
public class FareTable {
    ...
    private final Map<P2<String>, Fare> fares = Maps.newHashMap();
    ...
    public FareTable(String name) {
        ...
        try {
            ...
            while (reader.readRecord()) {
                ...
                this.fares.put(new P2<String>(from, to), fare);
            }
    ...
    public Fare lookup(String from, String to) {
        return new Fare(this.fares.get(new P2<String>(from, to))); // defensive copy, in case the
                                                                   // caller discounts
    }
