 ...
 * TransitStop.java and LocalStopFinder.java) as well as transfer limits, timed and preferred
 * transfer rules, transfer penalties, and boarding costs. This avoids applying these costs/rules
 * repeatedly in (Pattern)Board edges. These are single station or station-to-station specific
 * costs, rather than trip-pattern specific costs.
 */
public class PreBoardEdge extends FreeEdge implements StationEdge {
    ...
    public PreBoardEdge(TransitStop from, TransitStopDepart to) {
        super(from, to);
        if (!(from instanceof TransitStop)) { throw new IllegalStateException("Preboard edges must lead out of a transit stop."); }
    }
    ...
    public State traverse(State s0) {
        ...
        if (!options.bannedStops.isEmpty()) {
            if (options.bannedStops.matches(((TransitStop) this.fromv).getStop())) { return null; }
        }
        if (!options.bannedStopsHard.isEmpty()) {
            if (options.bannedStopsHard.matches(((TransitStop) this.fromv).getStop())) { return null; }
        }
    ...
    public String toString() {
        return "preboard edge at stop " + this.fromv;
    }
