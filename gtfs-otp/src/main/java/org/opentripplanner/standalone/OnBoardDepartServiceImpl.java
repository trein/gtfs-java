...
/**
 * Graph service for depart-on-board mode. Default implementation with initial position and
 * date-time set. We need departure date-time on top of initial position to determine the service
 * day on which the given trip is running. Works only for standard trips: frequency-based is not
 * (yet?) supported. TODO The method is not optimal for looping trips, as several origin point can
...
public class OnBoardDepartServiceImpl implements OnBoardDepartService {
    ...
    public Vertex setupDepartOnBoard(RoutingContext ctx) {
        ...
        if (point != null) {
            ...
            if (bestServiceDay == null) { throw new RuntimeException("Unable to determine on-board depart service day."); }
        ...
        OnBoardDepartPatternHop startHop = new OnBoardDepartPatternHop(onboardDepart, nextStop, bestTripTimes, bestServiceDay,
