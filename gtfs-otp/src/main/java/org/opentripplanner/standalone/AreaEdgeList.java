...
public class AreaEdgeList implements Serializable {
    ...
    private final HashSet<IntersectionVertex> vertices = new HashSet<IntersectionVertex>();
    ...
    public void setEdges(ArrayList<AreaEdge> edges) {
        ...
        for (AreaEdge edge : edges) {
            this.vertices.add((IntersectionVertex) edge.getFromVertex());
        }
    }
    ...
    public void removeEdge(AreaEdge edge) {
        ...
        for (Edge e : this.edges) {
            this.vertices.add((IntersectionVertex) e.getFromVertex());
        }
    }
    ...
    public void addVertex(IntersectionVertex newVertex, Graph graph) {
        ...
        if (this.edges.size() == 0) { throw new RuntimeException("Can't add a vertex to an empty area"); }
        
        @SuppressWarnings("unchecked")
        HashSet<IntersectionVertex> verticesCopy = (HashSet<IntersectionVertex>) this.vertices.clone();
        VERTEX: for (IntersectionVertex v : verticesCopy) {
            LineString newGeometry = geometryFactory.createLineString(new Coordinate[] { newVertex.getCoordinate(),
                    v.getCoordinate() });
            
            // ensure that new edge does not leave the bounds of the original area, or
            // fall into any holes
            if (!this.originalEdges.union(this.originalEdges.getBoundary()).contains(newGeometry)) {
                continue VERTEX;
            }
            ...
            createSegments(newVertex, v, this.areas, graph);
        }
    ...
    private void createSegments(IntersectionVertex from, IntersectionVertex to, List<NamedArea> areas, Graph graph) {
        ...
        if (intersects.size() == 1) {
            ...
            AreaEdge forward = new AreaEdge(from, to, line, area.getName(), length, area.getPermission(), false, 0, this);
            forward.setStreetClass(area.getStreetClass());
            AreaEdge backward = new AreaEdge(to, from, (LineString) line.reverse(), area.getName(), length, area.getPermission(),
                    true, 0, this);
