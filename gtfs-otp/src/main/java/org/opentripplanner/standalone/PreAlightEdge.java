 ...
 * agency_stop_arrive vertices to a TransitStop. Applies the local stop rules (see TransitStop.java
 * and LocalStopFinder.java) as well as transfer limits, timed and preferred transfer rules,
 * transfer penalties, and boarding costs. This avoids applying these costs/rules repeatedly in
 * (Pattern)Board edges. These are single station or station-to-station specific costs, rather than
...
public class PreAlightEdge extends FreeEdge implements StationEdge {
    ...
    public PreAlightEdge(TransitStopArrive from, TransitStop to) {
        super(from, to);
        if (!(to instanceof TransitStop)) { throw new IllegalStateException("Pre-alight edges must lead to a transit stop."); }
    }
    ...
    public State traverse(State s0) {
        ...
        if (!options.bannedStops.isEmpty()) {
            if (options.bannedStops.matches(((TransitStop) this.tov).getStop())) { return null; }
        }
        if (!options.bannedStopsHard.isEmpty()) {
            if (options.bannedStopsHard.matches(((TransitStop) this.tov).getStop())) { return null; }
        }
    ...
    public String toString() {
        return "prealight edge at stop " + this.tov;
    }
