...
public class ElevationGraphBuilderImpl implements GraphBuilder {
    ...
    public void setDistanceBetweenSamplesM(double distance) {
        this.distanceBetweenSamplesM = distance;
    }
    
    @Override
    public void buildGraph(Graph graph, HashMap<Class<?>, Object> extra) {
        this.gridCoverageFactory.setGraph(graph);
        Coverage gridCov = this.gridCoverageFactory.getGridCoverage();
        ...
        this.coverage = (gridCov instanceof GridCoverage2D) ? Interpolator2D.create((GridCoverage2D) gridCov,
                new InterpolationBilinear()) : gridCov;
    ...
    class ElevationRepairState {
        ...
        public ElevationRepairState(EdgeWithElevation backEdge, ElevationRepairState backState, Vertex vertex, double distance,
        ...
        while (!pq.empty()) {
            ...
            for (Edge e : state.vertex.getOutgoing()) {
                ...
                if (elevation != null) {
                    ...
                    // continue
                    ElevationRepairState newState = new ElevationRepairState(edge, state, tov, e.getDistance() + state.distance,
            ...
            if (bestDistance != Double.MAX_VALUE) {
                ...
                while (true) {
                    ...
                    if (totalDistance == 0) {
                        ...
                        double elevation = ((bestElevation * state.distance) + (state.initialElevation * bestDistance))
                                / totalDistance;
    ...
    private void processEdge(Graph graph, EdgeWithElevation ee) {
        if (ee.getElevationProfile() != null) { return; /* already set up */
        }
        ...
        for (int i = 0; i < (coords.length - 1); i++) {
            edgeLenM += this.distanceLibrary.distance(coords[i].y, coords[i].x, coords[i + 1].y, coords[i + 1].x);
        }
        ...
        for (double x = this.distanceBetweenSamplesM; x < edgeLenM; x += this.distanceBetweenSamplesM) {
            // avoid final-segment samples less than half the distance between samples:
            if ((edgeLenM - x) < (this.distanceBetweenSamplesM / 2)) {
                break;
            }
    ...
    public Coordinate getPointAlongEdge(Coordinate[] coords, double length, double t) {
        ...
        for (int i = 1; i < (coords.length - 1); i++) { // loop through inner points
            ...
            double pct = this.distanceLibrary.distance(y1, x1, y2, x2) / length;
    ...
    private double getElevation(double x, double y) {
        ...
        try {
            this.coverage.evaluate(new DirectPosition2D(x, y), values);
