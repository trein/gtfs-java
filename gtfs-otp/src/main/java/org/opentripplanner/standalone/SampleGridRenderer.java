...
import org.opentripplanner.standalone.AccumulativeGridSampler.AccumulativeMetric;
...
/**
 * Compute a sample grid from a SPT request. First compute a shortest-path-tree from the given
 * routing request. It then build the sample grid (a regular grid of samples covering the whole SPT
 * area) using an accumulative grid sampling process.
 *
 * @see ZSampleGrid
...
public class SampleGridRenderer {
    ...
    private final SPTService sptService;
    ...
    public ZSampleGrid<WTWD> getSampleGrid(SampleGridRequest spgRequest, RoutingRequest sptRequest) {
        ...
        sptRequest.worstTime = (sptRequest.dateTime + (sptRequest.arriveBy ? -spgRequest.maxTimeSec - tOvershot
                : spgRequest.maxTimeSec + tOvershot));
        sptRequest.batch = (true);
        sptRequest.setRoutingContext(this.graphService.getGraph(sptRequest.routerId));
        final ShortestPathTree spt = this.sptService.getShortestPathTree(sptRequest);
        ...
        SparseMatrixZSampleGrid<WTWD> sampleGrid = new SparseMatrixZSampleGrid<WTWD>(16, spt.getVertexCount(), dX, dY,
                coordinateOrigin);
    ...
    public static void sampleSPT(ShortestPathTree spt, ZSampleGrid<WTWD> sampleGrid, final double d0,
        ...
        AccumulativeMetric<WTWD> accMetric = new AccumulativeMetric<WTWD>() {
             ...
             * time. All this are approximations. TODO Is there a better way of computing this? Here
             * the computation will be different based on the order where we close the samples.
             */
            @Override
        ...
        johnny.walk(new SPTWalker.SPTVisitor() {
            @Override
    ...
    /**
     * The default TZ data we keep for each sample: Weighted Time and Walk Distance For now we keep
     * all possible values in the vector; we may want to remove the values that will not be used in
     * the process (for example # of boardings). Currently the filtering is done afterwards, it may
     * be faster and surely less memory-intensive to do the filtering when processing.
     *
     * @author laurent
     */
    public static class WTWD {
        ...
        public String toString() {
            return String.format("[t/w=%f,w=%f,d=%f]", this.wTime / this.w, this.w, this.d);
        }
