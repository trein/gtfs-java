...
public class Segment {
    ...
    public Segment(Ride ride) {
        ...
        for (PatternRide patternRide : ride.patternRides) {
            ...
            // LOG.info("   pattern {} {} from {} to {}", patternRide.pattern.mode,
            // patternRide.pattern.getCode(), patternRide.getFromStop(), patternRide.getToStop());
        }
        Collections.sort(this.segmentPatterns);
        // Note that despite the fact that multiple patterns from different routes will appear in
        // the same ride,
        // in practice all the patterns within a ride will be from the same operator and mode
