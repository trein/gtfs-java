 ...
 * http://jcommander.org/#Parameter_validation Some parameter fields are not initialized so when
 * inferring other parameters, we can check for null and see whether they were specified on the
 * command line.
 *
 * @author abyrd
 */
public class CommandLineParameters {
    ...
    public void infer() {
        ...
        if (this.graphDirectory == null) {
            this.graphDirectory = DEFAULT_GRAPH_DIRECTORY;
        }
        if (this.routerIds == null) {
            this.routerIds = Arrays.asList(DEFAULT_ROUTER_ID);
        }
        if (this.cacheDirectory == null) {
            this.cacheDirectory = DEFAULT_CACHE_DIRECTORY;
        }
        if (this.pointSetDirectory == null) {
            this.pointSetDirectory = DEFAULT_POINTSET_DIRECTORY;
        }
        if (this.server && (this.port == null)) {
            this.port = DEFAULT_PORT;
            new AvailablePort().validate(this.port);
        }
        if (this.server && (this.securePort == null)) {
            this.securePort = DEFAULT_SECURE_PORT;
            new AvailablePort().validate(this.securePort);
        }
    }
