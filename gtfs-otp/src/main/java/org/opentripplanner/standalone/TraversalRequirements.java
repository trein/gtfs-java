...
public class TraversalRequirements {
    ...
    /**
     * The maximum slope of streets for wheelchair trips. ADA max wheelchair ramp slope is a good
     * default.
     */
    ...
    /**
     * Default constructor. By default, accepts all modes of travel and does not require wheelchair
     * access.
     */
    ...
    public TraversalRequirements(RoutingRequest options) {
        ...
        if (bikeWalkOptions != null) {
            ...
            initFromRoutingRequest(this.bikeWalkingRequirements, bikeWalkOptions);
        }
    }
    ...
    private boolean canBeTraversedInternal(StreetEdge e) {
        if (this.wheelchairAccessible) {
            if (!e.isWheelchairAccessible()) { return false; }
            if (e.getElevationProfileSegment().getMaxSlope() > this.maxWheelchairSlope) { return false; }
        }

        return e.getPermission().allows(this.modes);
    }
    ...
    public boolean canBeTraversed(StreetEdge e) {
        if (canBeTraversedInternal(e)) {
            return true;
        } else if (hasBikeWalkingRequirements() && this.bikeWalkingRequirements.canBeTraversedInternal(e)) { return true; }
        return false;
    }
