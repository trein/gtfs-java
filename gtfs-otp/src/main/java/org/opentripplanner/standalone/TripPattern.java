 ...
 * non-overtaking, so that an earlier trip never arrives after a later trip. This is called a
 * JOURNEY_PATTERN in the Transmodel vocabulary. However, GTFS calls a Transmodel JOURNEY a "trip",
 * thus TripPattern.
 */
public class TripPattern implements Serializable {
     ...
     * minus 1. This also allow to access the ordered list of stops. This appears to only be used
     * for on-board departure. TODO: stops can now be grabbed from stopPattern.
     */
    ...
    public TripPattern(Route route, StopPattern stopPattern) {
         ...
         * pattern.
         */
        this.stopVertices = new TransitStop[size];
        this.departVertices = new PatternDepartVertex[size];
        this.arriveVertices = new PatternArriveVertex[size];
        this.boardEdges = new TransitBoardAlight[size];
        this.alightEdges = new TransitBoardAlight[size];
        this.hopEdges = new PatternHop[size];
        this.dwellEdges = new PatternDwell[size];
    }
    ...
    private void setStopsFromStopPattern(StopPattern stopPattern) {
        ...
        for (Stop stop : stopPattern.stops) {
            ...
            if (stop.getWheelchairBoarding() != 2) {
                this.perStopFlags[i] |= FLAG_WHEELCHAIR_ACCESSIBLE;
            }
            this.perStopFlags[i] |= stopPattern.pickups[i] << SHIFT_PICKUP;
            this.perStopFlags[i] |= stopPattern.dropoffs[i] << SHIFT_DROPOFF;
            ++i;
        }
    }

    public Stop getStop(int stopIndex) {
        if (stopIndex == this.patternHops.length) {
            return this.patternHops[stopIndex - 1].getEndStop();
        } else {
            return this.patternHops[stopIndex].getBeginStop();
        }
    }
    ...
    void setPatternHop(int stopIndex, PatternHop patternHop) {
        this.patternHops[stopIndex] = patternHop;
    }
    
    public Trip getTrip(int tripIndex) {
        return this.trips.get(tripIndex);
    }
    ...
    public boolean wheelchairAccessible(int stopIndex) {
        return (this.perStopFlags[stopIndex] & FLAG_WHEELCHAIR_ACCESSIBLE) != 0;
    }
    ...
    public int getAlightType(int stopIndex) {
        return (this.perStopFlags[stopIndex] & MASK_DROPOFF) >> SHIFT_DROPOFF;
    }
    
    public int getBoardType(int stopIndex) {
        return (this.perStopFlags[stopIndex] & MASK_PICKUP) >> SHIFT_PICKUP;
    }
    ...
    public void add(FrequencyEntry freq) {
        this.trips.add(freq.tripTimes.trip);
    ...
    public Timetable getUpdatedTimetable(RoutingRequest req, ServiceDay sd) {
        if ((req != null) && (req.rctx != null) && (req.rctx.timetableSnapshot != null) && (sd != null)) { return req.rctx.timetableSnapshot
     ...
     * have the same "unique" name? The names should be dataset unique, not just route-unique? A
     * TripPattern groups all trips visiting a particular pattern of stops on a particular route.
     ...
     * 59th St (or from Coney Island to 59th in the opposite direction). TripPattern names are
     * machine-generated on a best-effort basis. They are guaranteed to be unique (among
     * TripPatterns for a single Route) but not stable across graph builds, especially when
     * different versions of GTFS inputs are used. For instance, if a variant is the only variant of
     * the N that ends at Coney Island, the name will be "N to Coney Island". But if multiple
     ...
     * needed for shape distances (actually, stop sequence numbers?). TODO move GtfsStopContext into
     * Graph.
     */
    public void makePatternVerticesAndEdges(Graph graph, GtfsStopContext context) {
        ...
        int nStops = this.stopPattern.size;
        for (int stop = 0; stop < (nStops - 1); stop++) {
            Stop s0 = this.stopPattern.stops[stop];
            Stop s1 = this.stopPattern.stops[stop + 1];
            pdv0 = new PatternDepartVertex(graph, this, stop);
            this.departVertices[stop] = pdv0;
            if (stop > 0) {
                pav0 = pav1;
                this.dwellEdges[stop] = new PatternDwell(pav0, pdv0, stop, this);
            }
            pav1 = new PatternArriveVertex(graph, this, stop + 1);
            this.arriveVertices[stop + 1] = pav1;
            this.hopEdges[stop] = new PatternHop(pdv0, pav1, s0, s1, stop);
            ...
            if (this.canBoard(stop)) {
                this.boardEdges[stop] = new TransitBoardAlight(stopDepart, pdv0, stop, mode);
            }
            if (this.canAlight(stop + 1)) {
                this.alightEdges[stop + 1] = new TransitBoardAlight(pav1, stopArrive, stop + 1, mode);
            }
        }
    }
    ...
    public void dumpVertices() {
        for (int i = 0; i < this.stopPattern.size; ++i) {
            Vertex arrive = this.arriveVertices[i];
            Vertex depart = this.departVertices[i];
            System.out.format("%s %02d %s %s\n", this.code, i, arrive == null ? "NULL" : arrive.getLabel(),
    ...
    public void setServiceCodes(Map<AgencyAndId, Integer> serviceCodes) {
        ...
        for (Trip trip : this.trips) {
            this.services.set(serviceCodes.get(trip.getServiceId()));
        }
