...
/**
 * A Ride is defined by its from-stop and to-stop, as well as the previous ride (from which it
 * transfers). It serves as a container for multiple PatternRides, all of which connect that same
 * pair of Stops. A Ride may be unfinished, which means it does not yet have a destination and
 * contains only unfinished PatternRides. TODO this could just be a RouteRide key mapped to lists of
 * patternRides with the same characteristics. its constructor would pull out the right fields. TODO
 * if patternRides can be added later, and stats and transfers are computed after the fact, then
 * maybe we no longer need a round-based approach. We need to put _groups_ of patternRides into the
 * queue, grouped by back ride and beginning stop.
 */
public class Ride {
    ...
    int accessTime; // minimum time to reach this ride from the previous one, or from the origin
                    // point on the first ride
    ...
    public String toString() {
        return String.format("Ride from %s to %s (%d patterns on routes %s)", this.from, this.to, this.patternRides.size(),
                getRoutes());
    }
    ...
    public Multimap<Route, PatternRide> getPatternRidesByRoute() {
        ...
        for (PatternRide pr : this.patternRides) {
    ...
    public int durationLowerBound() {
        ...
        while (ride != null) {
            ...
            ret += ride.accessTime; // minimum access time to first ride, or transfer walking time
                                    // on subsequent ones.
            ride = ride.previous;
        }
        return ret;
    ...
    public int durationUpperBound() {
        ...
        while (ride != null) {
            ...
            ret += ride.accessTime; // FIXME using fastest access mode when calculating upper bound
                                    // time... is this OK?
            ride = ride.previous;
        }
        return ret;
    ...
    /**
     * Create a compound Stats for all the constituent PatternRides of this Ride. This should not be
     * called until all PatternRides have been added to this Ride.
     */
     ...
     * @return a list of sorted departure or arrival times within the window. FIXME this is a hot
     *         spot in execution, about 50 percent of runtime.
     */
    ...
    public Stats calcStatsForTransfer(TimeWindow window, double walkSpeed) {
        ...
        if (this.previous == null) { return this.statsForBoarding(window); }
        List<Integer> arrivals = this.previous.getSortedStoptimes(window, true);
        ...
        ARRIVAL: for (int arrival : arrivals) {
            int boardTime = arrival + this.accessTime + ProfileRouter.SLACK;
    ...
    /**
     * @return the stop from which the rider will walk to the final destination, assuming this is the
     *         final Ride in a chain.
     */
