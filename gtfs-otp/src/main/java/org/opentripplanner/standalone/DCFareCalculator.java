...
public class DCFareCalculator {
    ...
    private static final String[] metroExpress = { "J7", "J9", "P17", "P19", "W13", "W19", "11Y", "17A", "17B", "17G", "17H",
            "17K", "17L", "17M", "18E", "18G", "18H", "18P", "29E", "29G", "29H", "29X" };
    ...
    private static RideType classify(Route route) {
        ...
        String agency_url = route.getAgency().getUrl(); // this is used in single-agency feeds so it
                                                        // should work
        ...
        if ("MET".equals(agency)) {
            ...
            if (agency_url.contains("fairfaxconnector.com")) { return RideType.FAIRFAX_CONNECTOR_BUS; }
            if (agency_url.contains("prtctransit.org")) { return RideType.PRTC_BUS; }
            if (agency_url.contains("arlingtontransit.com")) { return RideType.ART_BUS; }
            if (agency_url.contains("vre.org")) { return RideType.VRE_BUS; }
    ...
    public static List<Fare> calculateFares(List<Ride> rides) {
        ...
        FareRide prev = null;
        for (Ride ride : rides) {
            // Calculate the fare for a ride based on only one of its patterns. This is usually
            // right as long as the
            // patterns are all the same mode and operator. That happens less naturally now that
            // stops are being grouped.
    ...
    static class FareRide {
        ...
        public FareRide(PatternRide pRide, FareRide prevRide) {
            this.from = pRide.getFromStop();
            this.to = pRide.getToStop();
            ...
            // This seems to happen naturally because different operators generally do not share
            // stops.
            this.route = pRide.pattern.route;
            this.type = classify(this.route);
            this.prev = prevRide;
            calcFare();
        }
        
        private void setFare(double base, boolean transferReduction) {
            this.fare = new Fare(base);
            this.fare.transferReduction = transferReduction;
        }
        
        private void setFare(double low, double peak, double senior, boolean transferReduction) {
            this.fare = new Fare(peak);
            this.fare.low = low;
            this.fare.senior = senior;
            this.fare.transferReduction = transferReduction;
        }
        ...
        private void calcFare() {
            RideType prevType = (this.prev == null) ? null : this.prev.type;
            if (this.type == null) { return; }
            switch (this.type) {
                case METRO_RAIL:
                    this.fare = METRORAIL.lookup(this.from, this.to);
                    if ((prevType == RideType.METRO_BUS_LOCAL) || (prevType == RideType.METRO_BUS_EXPRESS) || // TODO
                                                                                                              // merge
                                                                                                              // local
                                                                                                              // and
                                                                                                              // express
                                                                                                              // categories
                            (prevType == RideType.MCRO_BUS_LOCAL) || (prevType == RideType.MCRO_BUS_EXPRESS)) { // TODO
                                                                                                                // merge
                                                                                                                // local
                                                                                                                // and
                                                                                                                // express
                                                                                                                // categories
                        this.fare.discount(0.50);
                    }
                    break;
                ...
                case DASH_BUS:
                    ...
                    break;
                case MARC_RAIL:
                    this.fare = MARC.lookup(this.from, this.to);
                    break;
                case VRE_BUS:
                    this.fare = VRE.lookup(this.from, this.to);
                    break;
                case MCRO_BUS_LOCAL:
                ...
                case FAIRFAX_CONNECTOR_BUS:
                    String routeName = this.route.getShortName();
                    ...
                    break;
                case PRTC_BUS:
                    routeName = this.route.getLongName();
    ...
    public static class Fare {
        ...
        public Fare(double base) {
            this.low = this.peak = this.senior = base;
        }
