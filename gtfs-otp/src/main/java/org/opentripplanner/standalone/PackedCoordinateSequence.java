 ...
 * each time the coordinate sequence contents are modified through a setter method. 2009-11-25 -
 * bdferris - This class copied from JTS (LGPL-licensed) to add {@link Serializable} to the class so
 * that we can serialize it with our graphs
 *
 * @version 1.7
 */
public abstract class PackedCoordinateSequence implements CoordinateSequence, Serializable, Cloneable {
    ...
    public static class Double extends PackedCoordinateSequence {
        ...
        public Double(double[] coords, int dimensions) {
            if (dimensions < 2) { throw new IllegalArgumentException("Must have at least 2 dimensions"); }
            if ((coords.length % dimensions) != 0) { throw new IllegalArgumentException("Packed array does not contain "
