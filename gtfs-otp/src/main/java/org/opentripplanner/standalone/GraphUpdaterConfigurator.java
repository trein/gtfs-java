 ...
 * come from the graph itself or any other sources. When a graph is loaded, client should call
 * setupGraph() with the Preferences setup. When a graph is unloaded, one must ensure the
 * shutdownGraph() method is called to cleanup all resources that could have been created. This
 * class then creates "graph updaters" (usually real-time connector, etc...) depending on the given
 * configuration, and configure them using the corresponding children Preferences node. If an
 * embedded configuration is present in the graph, we also try to use it. In case of conflicts
...
public class GraphUpdaterConfigurator {
     ...
     */
    private GraphUpdaterManager applyConfigurationToGraph(Graph graph, GraphUpdaterManager updaterManager,
            List<Preferences> configs) {
        try {
