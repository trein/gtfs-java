...
import org.opentripplanner.standalone.Fare.FareType;
...
public class DefaultFareServiceImpl implements FareService, Serializable {
    ...
    public DefaultFareServiceImpl(HashMap<AgencyAndId, FareRuleSet> fareRules, HashMap<AgencyAndId, FareAttribute> fareAttributes) {
    ...
    public Fare getCost(GraphPath path) {
        ...
        if (this.fareAttributes.size() > 0) {
            currency = Currency.getInstance(this.fareAttributes.values().iterator().next().getCurrencyType());
    ...
    protected float calculateCost(List<Ride> rides) {
        ...
        if (bestFare == Float.POSITIVE_INFINITY) {
            if (this.fareAttributes.isEmpty()) {
