...
public class ProfileRouter {
    ...
    private static final List<TraverseMode> ACCESS_MODES = Lists.newArrayList(TraverseMode.WALK, TraverseMode.BICYCLE,
            TraverseMode.CAR);
    ...
    /* Search state */
    Multimap<StopCluster, StopAtDistance> fromStopPaths, toStopPaths; // ways to reach each origin
                                                                      // or dest stop cluster
    ...
    Collection<StopAtDistance> directPaths = Lists.newArrayList(); // ways to reach the destination
                                                                   // without transit
    Multimap<StopCluster, Ride> retainedRides = ArrayListMultimap.create(); // the rides arriving at
                                                                            // each stop that are
                                                                            // worth continuing to
                                                                            // explore.
    BinHeap<Ride> queue = new BinHeap<Ride>(); // rides to be explored, prioritized by minumum
                                               // travel time
    ...
    /**
     * @return true if the given stop cluster has at least one transfer coming from the given
     *         pattern.
     */
    private boolean hasTransfers(StopCluster stopCluster, TripPattern pattern) {
        for (ProfileTransfer tr : this.graph.index.transfersFromStopCluster.get(stopCluster)) {
    ...
    public ProfileResponse route() {
        ...
        if (this.request.analyst) {
            this.mins = new int[Vertex.getMaxIndex()];
            this.maxs = new int[Vertex.getMaxIndex()];
            Arrays.fill(this.mins, TimeSurface.UNREACHABLE);
            Arrays.fill(this.maxs, TimeSurface.UNREACHABLE);
        }
        LOG.info("modes: {}", this.request.modes);
        ...
        this.window = new TimeWindow(this.request.fromTime, this.request.toTime, this.graph.index
                .servicesRunning(this.request.date));
        ...
        if (!this.request.analyst) {
            ...
            for (TraverseMode mode : ACCESS_MODES) {
                if (this.request.modes.contains(mode)) {
        ...
        LOG.info("To patterns/stops: {}", this.toStops);
        
        /*
         * Enqueue an unfinished PatternRide for each pattern near the origin, grouped by Stop into
         * unfinished Rides.
         */
        Map<StopCluster, Ride> initialRides = Maps.newHashMap(); // One ride per stop cluster
        for (Entry<TripPattern, StopAtDistance> entry : this.fromStops.entrySet()) {
            TripPattern pattern = entry.getKey();
            StopAtDistance sd = entry.getValue();
            if (!this.request.modes.contains(pattern.mode)) {
                continue; // FIXME why are we even storing these patterns?
            }
            /*
             * Loop over stop clusters in case stop cluster appears more than once in the same
             * pattern.
             */
            for (int i = 0; i < pattern.getStops().size(); ++i) {
                // FIXME using String identity equality for stop clusters on purpose
                if (sd.stop == this.graph.index.stopClusterForStop.get(pattern.getStops().get(i))) {
                    Ride ride = initialRides.get(sd.stop);
                    if (ride == null) {
                        ride = new Ride(sd.stop, null); // null previous ride because this is the
                                                        // first ride
        ...
        while (!this.queue.empty()) {
            ...
            /*
             * Complete partial PatternRides (with only a pattern and a beginning stop) which were
             * enqueued in this partial ride. This is done by scanning through the Pattern, creating
             * rides to all downstream stops that are near the destination or have relevant
             * transfers.
             */
            PR: for (PatternRide pr : ride.patternRides) {
                ...
                for (int s = pr.fromIndex + 1; s < stops.size(); ++s) {
                    StopCluster cluster = this.graph.index.stopClusterForStop.get(stops.get(s));
                    /*
                     * Originally we only extended rides to destination stops considered useful in
                     * the search, i.e. those that had transfers leading out of them or were known
                     * to be near the destination. However, analyst needs to know the times we can
                     * reach every stop, and pruning is more effective if we know when rides pass
                     * through all stops.
                     */
                    PatternRide pr2 = pr.extendToIndex(s, this.window);
            ...
            Map<StopCluster, Ride> xferRides = Maps.newHashMap(); // A map of incomplete rides after
                                                                  // transfers, one for each stop.
            for (Ride r1 : rides.values()) {
                r1.calcStats(this.window, this.request.walkSpeed);
                if (r1.waitStats == null) {
                    // This is a sign of a questionable algorithm: we're only seeing if it was
                    // possible
                ...
                if (!addIfNondominated(r1)) {
                    continue; // abandon this ride if it is dominated by some existing ride at the
                              // same location
                }
                // We have a new, nondominated, completed ride. Record its lower and upper bounds at
                // the arrival stop.
                if (this.request.analyst) {
                    // This could be done at the end now that we are retaining all rides.
                    TransitStop tstop = this.graph.index.stopVertexForStop.get(r1.to);
                    int tsidx = tstop.getIndex();
                    int lb = r1.durationLowerBound();
                    int ub = r1.durationUpperBound();
                    if ((this.mins[tsidx] == TimeSurface.UNREACHABLE) || (this.mins[tsidx] > lb)) {
                        this.mins[tsidx] = lb;
                    }
                    if ((this.maxs[tsidx] == TimeSurface.UNREACHABLE) || (this.maxs[tsidx] > ub)) {
                        this.maxs[tsidx] = ub;
                    }
                }
                /* Find transfers out of this new ride. */
                // Do not transfer too many times. Check after calculating stats since stats are
                // needed in any case.
                ...
                for (ProfileTransfer tr : this.graph.index.transfersFromStopCluster.get(r1.to)) {
                    if (!this.request.modes.contains(tr.tp2.mode)) {
                        continue;
                    }
                    if (r1.containsPattern(tr.tp1)) {
                        ...
                        if (!this.request.analyst && penultimateRide && !this.toStops.containsKey(tr.tp2)) {
                            continue;
                        }
                        // Scan through stops looking for transfer target: stop might appear more
                        // than once in a pattern.
                        TARGET_STOP: for (int i = 0; i < tr.tp2.getStops().size(); ++i) {
                            StopCluster cluster = this.graph.index.stopClusterForStop.get(tr.tp2.getStops().get(i));
                            if (cluster == tr.sc2) {
                                ...
                                if (r2 == null) {
                                    ...
                                    r2.accessTime = (int) (tr.distance / this.request.walkSpeed);
                                    xferRides.put(tr.sc2, r2);
                                }
                                for (PatternRide pr : r2.patternRides) {
                                    // Multiple patterns can have transfers to the same target
                                    // pattern,
            ...
            for (Ride r : xferRides.values()) {
                ...
                if (addIfNondominated(r)) {
                    this.queue.insert(r, r.previous.durationLowerBound());
                }
            }
        ...
        Set<Ride> targetRides = Sets.newHashSet(); // FIXME determine why there are multiple copies
                                                   // of the same ride then maybe use a list
        for (StopCluster cluster : this.toStopPaths.keySet()) {
            for (Ride ride : this.retainedRides.get(cluster)) {
                PATTERN: for (PatternRide pr : ride.patternRides) {
                    StopAtDistance clusterForPattern = this.toStops.get(pr.pattern);
        ...
        for (Ride ride : targetRides) {
            ...
            int dist = this.toStops.get(ride.patternRides.get(0).pattern).etime; // TODO time vs
                                                                                 // dist
            Collection<StopAtDistance> accessPaths = this.fromStopPaths.get(ride.getAccessStopCluster());
            Collection<StopAtDistance> egressPaths = this.toStopPaths.get(ride.getEgressStopCluster());
        ...
        options.add(new Option(null, this.directPaths, null));
        /*
         * for (Stop stop : graph.index.stopVertexForStop.keySet()) { TransitStop tstop =
         * graph.index.stopVertexForStop.get(stop); int min = mins[tstop.getIndex()]; int max =
         * maxs[tstop.getIndex()]; if (min == Integer.MAX_VALUE) LOG.info("{} unreachable",
         * tstop.getName()); else LOG.info("{} min {} max {}", tstop.getName(), min, max); }
         */
        return new ProfileResponse(options, this.request.orderBy, this.request.limit);
    }
    
    /**
     * Check whether a new ride has too long a duration relative to existing rides at the same
     * location or global time limit.
     */
    public boolean addIfNondominated(Ride newRide) {
        StopCluster cluster = newRide.to;
        if (cluster == null) { // if ride is unfinished, calculate time to its from cluster based on
                               // previous ride
        ...
        for (Ride oldRide : this.retainedRides.get(cluster)) {
            if (oldRide.to == null) {
                oldRide = oldRide.previous; // rides may be unfinished
            }
            // new ride must be strictly better (min and max) than any existing one with less
            // transfers.
            // this avoids inserting extra rides "just for fun".
            if ((oldRide.pathLength() < newRide.pathLength()) && (oldRide.durationLowerBound() < newRide.durationLowerBound())
                    && (oldRide.durationUpperBound() < newRide.durationUpperBound())) { return false; }
            // State is not strictly dominated. Perhaps it has the same number of transfers.
            // In this case we want to keep it as long as it's sometimes better than all the others
            // (time ranged overlap).
            if (newRide.durationLowerBound() > oldRide.durationUpperBound()) { return false; }
        }
        this.retainedRides.put(cluster, newRide);
        return true; // No existing ride is strictly better than the new ride.
    }
    
    /**
     * @param stopClusters a multimap from stop clusters to one or more StopAtDistance objects at
     *        the corresponding cluster.
     * @return for each TripPattern that passes through any of the supplied stop clusters, the stop
     *         cluster that is closest to the origin or destination point according to the distances
     *         in the StopAtDistance objects. In short, take a bunch of stop clusters near the
     *         origin or destination and return the quickest way to reach each pattern that passes
     *         through them. We want stop cluster references rather than indexes within the patterns
     *         because when a stop cluster appears more than once in a pattern, we want to consider
     *         boarding or alighting from that pattern at every index where the cluster occurs.
     */
    public Map<TripPattern, StopAtDistance> findClosestPatterns(Multimap<StopCluster, StopAtDistance> stopClusters) {
        ...
        for (StopAtDistance stopDist : stopClusters.values()) {
            for (Stop stop : stopDist.stop.children) {
                for (TripPattern pattern : this.graph.index.patternsForStop.get(stop)) {
    ...
    private Collection<StopAtDistance> findClosestStops(final TraverseMode mode, boolean dest) {
        ...
        rr.from = (new GenericLocation(this.request.from.lat, this.request.from.lon));
        // FIXME requires destination to be set, not necesary for analyst
        rr.to = new GenericLocation(this.request.to.lat, this.request.to.lon);
        rr.setArriveBy(dest);
        rr.setMode(mode);
        // TODO CAR does not seem to work. rr.setModes(new TraverseModeSet(TraverseMode.WALK,
        // mode));
        rr.setRoutingContext(this.graph);
        // Set batch after context, so both origin and dest vertices will be found.
        rr.batch = (true);
        rr.walkSpeed = this.request.walkSpeed;
        // RR dateTime defaults to currentTime.
        // If elapsed time is not capped, searches are very slow.
        long worstElapsedTime = this.request.accessTime * 60; // convert from minutes to seconds
        if (dest) {
            worstElapsedTime *= -1;
        }
        ...
        // Save the routing context for later cleanup. We need its temporary edges to render street
        // segments at the end.
        this.routingContexts.add(rr.rctx);
    ...
    static class StopFinderTraverseVisitor implements TraverseVisitor {
        ...
        public void visitVertex(State state) {
            ...
            if (vertex instanceof TransitStop) {
                ...
                if (this.stopClustersFound.containsKey(sd.stop)) { return; // record only the
                                                                           // closest stop in each
                                                                           // cluster
                }
                LOG.debug("found stop cluster: {}", sd);
                this.stopClustersFound.put(sd.stop, sd);
            }
        }
    }
    ...
    private void findStreetOption(TraverseMode mode) {
        ...
        int worstElapsedTime = this.request.streetTime * 60;
        rr.worstTime = (rr.dateTime + worstElapsedTime);
        rr.walkSpeed = this.request.walkSpeed;
        rr.bikeSpeed = this.request.bikeSpeed;
        GenericAStar astar = new GenericAStar();
        rr.setNumItineraries(1);
        ...
        if (state != null) {
            ...
            this.directPaths.add(new StopAtDistance(state));
        }
        this.routingContexts.add(rr.rctx); // save context for later cleanup so temp edges remain
                                           // available
    }
    
    // Major change: This needs to include all stops, not just those where transfers occur or those
    ...
    public P2<TimeSurface> makeSurfaces() {
        ...
        int worstElapsedTime = this.request.accessTime * 60; // convert from minutes to seconds
        rr.worstTime = (rr.dateTime + worstElapsedTime);
        rr.batch = (true);
        GenericAStar astar = new GenericAStar();
        rr.setNumItineraries(1);
        for (TransitStop tstop : this.graph.index.stopVertexForStop.values()) {
            int index = tstop.getIndex();
            // Generate a tree outward from all stops that have been touched in the basic profile
            // search
            if ((this.mins[index] == TimeSurface.UNREACHABLE) || (this.maxs[index] == TimeSurface.UNREACHABLE)) {
                continue;
            }
            rr.setRoutingContext(this.graph, tstop, null); // Set origin vertex directly instead of
                                                           // generating link edges
            astar.setTraverseVisitor(new ExtremaPropagationTraverseVisitor(this.mins[index], this.maxs[index]));
            ShortestPathTree spt = astar.getShortestPathTree(rr, 5);
            rr.rctx.destroy();
        }
        this.minSurface = new TimeSurface(this, false);
        this.maxSurface = new TimeSurface(this, true);
        LOG.info("done making timesurfaces.");
        return new P2<TimeSurface>(this.minSurface, this.maxSurface);
    }
    
    /**
     * Given a minimum and maximum at a starting vertex, build an on-street SPT and propagate those
     * values outward.
     */
    class ExtremaPropagationTraverseVisitor implements TraverseVisitor {
        ...
        @Override
        public void visitVertex(State state) {
            int min = this.min0 + (int) state.getElapsedTimeSeconds();
            int max = this.max0 + (int) state.getElapsedTimeSeconds();
            Vertex vertex = state.getVertex();
            int index = vertex.getIndex();
            if (index >= ProfileRouter.this.mins.length) { return; // New temp vertices may have
                                                                   // been created since the array
                                                                   // was dimensioned.
            }
            if ((ProfileRouter.this.mins[index] == TimeSurface.UNREACHABLE) || (ProfileRouter.this.mins[index] > min)) {
                ProfileRouter.this.mins[index] = min;
            }
            if ((ProfileRouter.this.maxs[index] == TimeSurface.UNREACHABLE) || (ProfileRouter.this.maxs[index] > max)) {
                ProfileRouter.this.maxs[index] = max;
            }
        }
    }
    ...
    public int cleanup() {
        int n = 0;
        for (RoutingContext rctx : this.routingContexts) {
            rctx.destroy();
            n += 1;
        }
