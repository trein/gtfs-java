...
public class SPTWalker {
    ...
    public void walk(SPTVisitor visitor, double d0) {
        ...
        Collection<? extends State> allStates = this.spt.getAllStates();
        ...
        for (State s0 : allStates) {
            for (Edge e : s0.getVertex().getIncoming()) {
                ...
                if ((e != null) && visitor.accept(e)) {
                    State s1 = this.spt.getState(e.getFromVertex());
                    if (s1 == null) {
                        continue;
                    }
                    ...
                    double lineStringLen = this.distanceLibrary.fastLength(lineString);
                    ...
                    if (lineStringLen > d0) {
                        ...
                        for (int i = 0; i < (pList.length - 1); i++) {
                            ...
                            double segLen = this.distanceLibrary.fastDistance(p0, p1);
        ...
        LOG.info("SPTWalker: Generated {} points ({} dup edges, {} no geometry) from {} states.", nTotal, nSkippedDupEdge,
                nSkippedNoGeometry, allStates.size());
    }
}
